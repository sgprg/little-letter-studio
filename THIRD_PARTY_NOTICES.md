...
The native Android and iOS shell templates originate from Capacitor (MIT). Capacitor Core, Android, iOS, Filesystem and Share are MIT-licensed, copyright 2017–present Drifty Co. and/or 2020–present Ionic, according to each bundled license. Filesystem also depends on Ionic's `ion-ios-filesystem` and `ion-android-filesystem` libraries (MIT, copyright 2025 Ionic); their full licenses are included. Full notices for installed runtime packages are copied into `public/third-party/` and included in the built app. The native bridge also uses Apache Cordova components under Apache-2.0. AndroidX runtime libraries are under Apache-2.0. The Gradle wrapper is Apache-2.0; build tools keep their respective licenses.
